...
- **ReadOnlyGuard ordering before live diagnostic service integration** — Cold / Trigger-based：Phase 2 可先建立 host-testable ISO-TP transport core，但不得因此形成 application/firmware 可直接送出任意 diagnostic request 的 production path。當 Phase 2 core 準備接入 Phase 3 OBD-II 或任何 live diagnostic service TX 時，先 reconcile `docs/READ_ONLY_POLICY.md` 與 Architecture Freeze 的要求，必要時將最小 `ReadOnlyGuard` enforcement Stage 提前 admission；所有 diagnostic TX 在 vehicle-facing integration 前必須經 allowlist guard。未 promotion 前不可執行。
