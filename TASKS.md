...
以下 Prompt 是為上述未完成項目預先保存的執行方案。**每次只能在使用者明確授權該 Stage 後執行該 Stage；不得因為讀到後續 Stage 就提前處理。** 每個 Stage 開始時都先讀 `AGENTS.md` 與根目錄 `TASKS.md`，並以當下 `main` 為 source of truth，不依賴本聊天室舊 SHA。

完成一個 Stage 後：先完成 targeted validation；只有驗證成功才更新 `TASKS.md`，移除已完成項目或更新 blocker/deferred 狀態。不要把完成紀錄留在 `TASKS.md`；Git history 才是完成紀錄。
...
1. AGENTS.md
2. 根目錄 TASKS.md
3. 僅在需要確認 architecture wording 時再讀 docs/ARCHITECTURE.md

請在 AGENTS.md 加入兩組永久規則：
...
可依本專案文件語言風格補充繁體中文說明，但不得改變上述語意。

B. Library-ready design
...
成功後同步更新 TASKS.md：移除「更新 AGENTS.md 的 TASKS.md 永久規則」與「補上 library-ready design 原則」兩個已完成項目。其他 TASKS 不得更動，除非只為保持文字一致所必須。

回報使用繁體中文，列出：變更檔案、規則摘要、validation、commit SHA。不要建立 PR，除非環境政策強制要求。
```
...
先讀 AGENTS.md 與 TASKS.md。以當下 main 為 source of truth，先確認 git status / branch / HEAD；若 working tree 有來源不明修改，停止回報。
...
- evidence 必須能證明 `Esp32TwaiCan` backend 實際參與編譯，而非只有 `src.ino`。
- 若 host tests 因 layout 變動受影響，也跑 targeted host build/test。

若最小修正會迫使大規模 build-system 重設計、跨 framework 遷移或無法證明 Arduino semantics，停止並回報 evidence，不自行擴張 scope。

成功後同步更新 TASKS.md：移除 P0 build-layout task；若 ESP32 backend CI coverage 已因此變得可直接實作，可將 Deferred 項目改為明確下一步，但本 Stage 不要自行新增大型 CI 系統。
...
**Context 建議：** Level 0→3；`can_types.h`、`can_hal.h`、`mock_can.h`、`test_helpers.h`、`tests/host/main.cpp` 與直接 caller。  
...
先讀 AGENTS.md、TASKS.md，再依 progressive expansion 讀：CanFrame model、CanHal contract、MockCan、test helpers、host tests 與必要 direct caller。
...
設計要求：
- 不要為了 invalid frame 新增過度複雜 error hierarchy；若現有 `CanStatus` 缺乏完美名稱，採最小一致方案並在回報說明 semantic debt。
...
成功後同步更新 TASKS.md：移除 CAN ID range、host helper malformed safety、MockCan lifecycle 三個完成項目。不要移除尚未由本 Stage 處理的 TWAI/RTR/BUS_OFF/overflow tasks。
...
**推薦模型：** Luna  
**推理強度：** Medium  
**推薦理由：** 主要 root causes 已由 ChatGPT/ESP-IDF evidence 定位，預期只需最小 HAL hardening；先用 Luna 最省成本。  
**是否值得先用較便宜模型做前置蒐證：** 否；不要重新做 repository-wide 或 upstream discovery。只有 local evidence 不足時才讀對應 Arduino-ESP32 / ESP-IDF 版本 source。  
...
先讀 AGENTS.md、TASKS.md 與 Stage 3 完成後的 current main。依 progressive expansion 只讀必要 HAL / model / tests。
...
已知 evidence 可先視為 investigation lead，但仍以 current toolchain/source 驗證：
- TWAI Classic CAN data buffer 為 8 bytes。
- TWAI format path會 mask 11-bit/29-bit IDs，因此 Stage 3 應已在 generic boundary 擋 invalid ID。
- legacy TWAI BUS_OFF 與 stop/uninstall state semantics不同。
- default TWAI RX queue 很小且目前 accept-all / alerts none。

升級條件：
若第 4 或第 5 項無法用最小 patch 完成，而必須設計跨模組 recovery state machine、FreeRTOS concurrency/backpressure、ISO-TP-aware scheduling，立即停止，不自行擴張 scope。將 root cause、需要的 state transitions、涉及模組與最小待決策問題寫入回報，改用下方 Terra fallback Stage 4B。

Validation：
- ESP32-S3 backend compile。
...
成功後同步更新 TASKS.md：移除 DLC boundary、Esp32TwaiCan ownership、RTR policy、BUS_OFF semantics、RX overflow observability 中已真正完成並驗證的項目；若某項僅部分完成，改寫 blocker/remaining work，不可假裝完成。

回報繁體中文：Evidence、Root Cause 分類、Focused Patch、Validation、Pending hardware evidence、commit SHA。
```

## Stage 4B — 條件式 Terra fallback（只有 Stage 4 明確觸發才用）

**推薦模型：** Terra  
**推理強度：** Medium  
**推薦理由：** 只有 Luna 證明最小 HAL patch 不足、問題實際牽涉 runtime state machine / concurrency 時才需要較高階推理。  
**是否值得先用較便宜模型做前置蒐證：** 是；Stage 4 Luna 的 evidence package 就是必要前置蒐證，禁止 Terra 重新從 repository-wide discovery 開始。  
**Context 建議：** 只載入 Stage 4 回報的 evidence、直接 HAL/state modules、必要 ESP-IDF semantics。  
**Execution mode：** Evidence-constrained design/patch。

### Codex Prompt

```text
只有在 Stage 4 已明確回報「最小 Luna patch 不足」時才執行本 Stage。若沒有 Stage 4 的具體 evidence / blocker，停止，不要自行開始。

先讀 AGENTS.md、TASKS.md、Stage 4 產生的 current repository state 與其 evidence。不要重新做 repository-wide discovery。
...
- 若真正需要跨模組 FreeRTOS task architecture 或大規模 API 改造，先停止並提出設計決策，不直接實作。

完成後做 targeted validation，並只更新 TASKS.md 中本 Stage 實際完成的對應項目。
...
**是否值得先用較便宜模型做前置蒐證：** 否；前四個 Stage 已提供完整 evidence。  
...
先讀 AGENTS.md、TASKS.md 與 current main。確認 Stage 1～4 已完成的 commit/state；不要假設舊聊天室 SHA。
...
- 其他已由前面 Stage 完成但仍殘留的 task，只有在你能從 current main + validation 證實完成時才移除。
